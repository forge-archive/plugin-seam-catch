...
public class HandlerPlugin implements Plugin {
     ...
     */
    @Inject
    public HandlerPlugin(final Project project, final Shell shell, final ShellPrintWriter writer, final ShellPrompt prompt) {
    ...
    public void handlerCommand(
        ...
        try {
            ...
            if (!handlerContainerClass.hasAnnotation(HandlesExceptions.class)) {
                writer.renderColor(ShellColor.RED, "This class is not an Exception Handler Container (it must be annotated with @HandlesExceptions)");
                return;
            }
